...
if [[ "$(uname -m)" != "aarch64" ]]; then
  echo "the OCI Always Free bundle requires an AArch64 host" >&2
  exit 2
fi
...
for command in caddy curl jq sha256sum systemctl; do
...
jq -e --arg sha "$EXPECTED_SHA" --arg revision "$EXPECTED_REVISION" \
  '.schemaVersion == 1 and
   .artifact == "castalia-bootstrap-node-linux-aarch64" and
   ...
   .build.target == "aarch64-unknown-linux-gnu" and
   .revision == $revision' \
...
# Validate the exact public route boundary before stopping a healthy node.
...
if [[ "$START_OK" -eq 1 ]]; then
  if "$SCRIPT_DIR/wait-for-verified-node.sh" \
    "https://${DREGG_HOSTNAME}/status" 900 >/dev/null; then
    READY=1
  fi
fi
...
if [[ -n "$PREVIOUS_TARGET" && -x "$PREVIOUS_TARGET" ]]; then
  ...
  if "$SCRIPT_DIR/wait-for-verified-node.sh" \
    ...
    echo "rolled back to healthy verified release $PREVIOUS_TARGET" >&2
  else
    echo "rollback release also failed verified readiness: $PREVIOUS_TARGET" >&2
  fi
fi
exit 1
